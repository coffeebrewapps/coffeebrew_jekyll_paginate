...
CONTEXT_FILENAME_CONFIG = "when different permalink and index_page configured"

RSpec.shared_context CONTEXT_FILENAME_CONFIG do
  let(:scenario) { "filename_config" }
  let(:overrides) do
    {
      "coffeebrew_jekyll_paginate" => {
        "collections" => {
          "posts" => {
            "permalink" => "/:page_num_one_index",
  ...
  let(:expected_files) do
    [
      ...
      dest_dir("1", "posts.html"),
      dest_dir("2", "posts.html"),
